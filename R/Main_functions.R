...
Calphad_2.5 <- function(ele,CP100,CP200,CP298,S100,S200,S298, env = Calphad.globals){
  ...
  Calphad.globals$ele <- ele
    ...
    knitr::kables(
      list(
        knitr::kable(results_Cp_S, col.names = c('Quantity', 'Input value', 'Model Result'), valign = 't',format = 'markdown'),
        knitr::kable(Results_output_parameters, col.names = c('Parameter', 'Value'),digits = 5, valign = 't',format = 'markdown')
