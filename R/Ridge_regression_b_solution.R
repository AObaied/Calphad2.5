...
#' Ridge_b_Solution
#'
#' @param Td_input parameter
#'
#' @importFrom data.table data.table
#' @importFrom stats lm
#' @importFrom stats coef
#'
#' @return Ridge_b_Solution
#'
Ridge_b_Solution <- function(Td_input){
  ...
  cp298 <- Calphad.globals$NOA*Debye_constant(Calphad.globals$RT,Td_input)
  s298 <- integrate(Debye_constant_DOT, 0.0001,Calphad.globals$RT)$val
  S_Debye_full <- s298
  #-------------------------------------------------------
  # Creat a table (dt) for T and Cp values along the Temp. range
  dt <- data.table(x = x, y = Calphad.globals$NOA*Debye_constant(x,Td_input))
  ...
  dt_cpot <- data.table(x = x, y = Calphad.globals$NOA*Debye_constant(x,Td_input)/x)
  ...
  # a2 coefficient function, used to insure a continuous curve at the break point
  ...
  errorcp298 <- function(b,c,d,e,f,g) {
    #f2p2 <- f2(Calphad.globals$bp,b,c,d,e)
